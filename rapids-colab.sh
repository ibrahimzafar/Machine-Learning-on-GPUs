...
echo "********************************************************************************************************"
echo "Changes:"
echo "1. IMPORTANT CHANGES: RAPIDS on Colab will be pegged to 0.17 Stable until further notice.  This version of RAPIDS, while works, is outdated.  We have alternative solutions, https://app.blazingsql.com, to run the latest versions of RAPIDS"
...
install_RAPIDS () {
	...
	PATH=$PATH:/usr/local/lib/python3.6/site-packages
	
        #pin python$PYTHON_VERSION
        ...
        # install RAPIDS packages
            conda install -y --prefix /usr/local \
                    ...
                    python=$PYTHON_VERSION gdal=3.0.4 cudatoolkit=$CTK_VERSION \
            ...
            conda install -y --prefix /usr/local \
                ...
                python=$PYTHON_VERSION cudatoolkit=$CTK_VERSION \
        ...
        elif (( $RAPIDS_RESULT == 17 )) ;then
            echo "Installing RAPIDS $RAPIDS_VERSION packages from the stable release channel"
            echo "Please standby, this will take a few minutes..."
            # install RAPIDS packages
            conda install -y --prefix /usr/local \
                -c rapidsai/label/main -c rapidsai -c nvidia -c conda-forge -c defaults \
                python=$PYTHON_VERSION gdal llvm cudatoolkit=$CTK_VERSION \
                rapids-blazing=$RAPIDS_VERSION gcsfs pynvml dask-cudf tensorflow-gpu-2.2
        fi
